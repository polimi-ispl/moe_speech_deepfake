...
def main(model_path, save_folder, config):
    ...
    expert_1 = LCNN(return_emb=True)
    expert_2 = LCNN(return_emb=True)
    expert_3 = LCNN(return_emb=True)
    expert_4 = LCNN(return_emb=True)
    ...
    moe_model = Enhanced_MOE(experts=[expert_1, expert_2, expert_3, expert_4], freezing=False)
    ...
    for dataset, df_eval in zip(['asvspoof', 'fakeorreal', 'inthewild', 'purdue', 'timit', 'add2022'], [df_eval_asv, df_eval_for, df_eval_itw, df_purdue, df_timit, df_eval_ADD]):
        ...
        eval_model(moe_model, eval_loader, result_path, 'cuda')
...
if __name__ == '__main__':
    ...
    model_path = os.path.join(config['save_model_folder'], config['moe_model_name'])
